...
alloc_block(void)
{
	...
	for (i = 0; i < super->s_nblocks; i++)
		if (block_is_free(i))
		{
			uint64_t addr = DISKMAP + i * BLKSIZE;
			flush_block((void *)addr);
			bitmap[i / 32] &= (uint32_t)(-1) & (0 << (i % 32));
			return i;
		}
	return -E_NO_DISK;
}
...
fs_init(void)
{
	...
	static_assert(sizeof(struct inode) == 256);
	...
	// Lab 5 - Challenge 4
	ibitmap = diskaddr(3);
	itable = (struct inode *) diskaddr(4);
}
...
static int
dir_lookup(struct inode *dir, const char *name, struct inode **file)
{
	int r;
	uint32_t i, j, nblock;
	char *blk;
...
static int
dir_alloc_file(struct inode *dir, struct File **file)
{
	int r;
	uint32_t nblock, i, j;
	char *blk;
...
static int
walk_path(const char *path, struct inode **pdir, struct inode **pf, char *lastelem)
{
	const char *p;
...
// On error return < 0.
int
file_open(const char *path, struct inode **pf)
{
